...
async def fermi_task(plc_connection: PLCConnection, ip: str, slot: int):
    ...
    if not trigger_conf:
        logging.warning(f"⚠️ No dati_pronti_fermi config for PLC {ip}:{slot}")
        return
    if not clock_conf:
        logging.warning(f"⚠️ No keepLive_fermi_PC config for PLC {ip}:{slot}")
    ...
    while True:
        try:
            if not plc_connection.connected or not plc_connection.is_connected():
                print(f"⚠️ PLC disconnected for {ip}:{slot}, attempting reconnect...")
            ...
            logging.error(f"[{ip}:{slot}] 🔴 Error in fermi_task: {e}")
            await asyncio.to_thread(plc_connection.reconnect, retries=3, delay=5)
