        ...
        try:
            ...
            if (fine_buona or fine_scarto) and not passato_flags[station]:
                ...
                if result:
                    ...
                    fb_conf = CHANNELS[station]["fine_buona"]
                    fs_conf = CHANNELS[station]["fine_scarto"]
                    fine_buona = await asyncio.to_thread(plc_connection.write_bool, fb_conf["db"], fb_conf["byte"], fb_conf["bit"], False)
                    fine_scarto = await asyncio.to_thread(plc_connection.write_bool, fs_conf["db"], fs_conf["byte"], fs_conf["bit"], False)
